//
//  This project is an interactive story heavily inspired by the animated famous Disney and Pixar movie "Toy Story".
//  Featuring the characters Woody, Jessie, Alien, Mr. Potatohead, and Lotso.
//  Our goal is to create a captivating narrative that highlights the unbreakable bonds of friendship.
//  Each team member was assigned a specific character and scene.
// 
//  Created by Aseel Keleeb, Budur Alghamdi, Noran Almughamisi, Teif Alhrbi, and Nrdeen Sahrah on 21/09/2023.
//  Students at Umm Al-Qura University (UQU) majoring in Computer Science.
//  This 'Computer Graphics' project was completed under the supervision of Dr.Omniah Nagoor on 21/10/2023.
...
//-------------TEXTURE PART-----------//
GLuint myTexture1;
char  image1Path[] = "C:\\Users\\as1ke\\Desktop\\College\\6th semester\\Computer Graphics\\Project\\woodenTexture.bmp";
...
// functions that are dervied from the original drawFilledCircle() function
...
/*
 * Function that handles the drawing of a text/string inside the clipping window
 * Params:
 *    font (void*) - the specific font and size of text. available fonts guide: //https://www.opengl.org/resources/libraries/glut/spec3/node76.html
 *    string (const char*) - the string to print on window
 *    x (float) - the x position of the text
 *    y (float) - the y position of the text
 *    function Source: https://www.openglprojects.in/2012/04/tutorial-how-to-display-strings-in.html#gsc.tab=0
 */
...
// the NEXT button used to move from one scene to another
void Button() {
    glBegin(GL_QUADS);
...
void Jessie() {
    ...
    drawHalfdownCircle(0.0, -0.1, -0.075);
...
void woody() {
    ...
    drawHalfdownCircle(0.0f, 0.17f, 0.13);
    //bottom
    glLineWidth(18);
    glBegin(GL_LINES);
    glColor3f(0.8f, 0.5f, 0.1f);
    ...
    // mouth
    glColor3f(0.8f, 0.4f, 0.4f);
    drawHalfdownCircle(0.0, -0.15, -0.075);
    //pants
...
void MrPootatoHead() {
    ...
    drawHalfdownCircle(0.08, 0.06, 0.06);
    glPopMatrix();
    glPushMatrix();
    glScalef(0.7 * 1, 0.7 * 1.3, 1.0);
    glTranslatef(0 - 0.8, 0, 0);
    ...
    drawHalfdownCircle(0, -0.17, 0.04);
    glPopMatrix();
    glPushMatrix();
    glColor3f(1, 1, 1);
    glRotatef(180, 1, 0, 0);
    ...
    drawHalfdownCircle(0, -0.17, 0.02);
    glPopMatrix();
    //mustache
    glPushMatrix();
    glColor3f(0, 0, 0);
    ...
    drawHalfdownCircle(0, -0.135, 0.065);
    glPopMatrix();
    glPushMatrix();
    glScalef(0.7 * 1, 0.7 * 1, 1);
    glTranslatef(0 - 0.8, 0, 0);
    ...
    glTranslatef(0 - 0.8, 0, 0);
    glColor3f(0, 0, 0);
    drawHalfdownCircle(0, 0.23, 0.13);
    glPopMatrix();
    glLineWidth(12);
    glPushMatrix();
    glScalef(0.7 * 1, 0.7 * 1, 1);
    ...
    glTranslatef(0 - 0.8, 0, 0);
    glColor3f(0, 0.5, 0.8);
    drawHalfdownCircle(0.08, -0.3, 0.08);
    glPopMatrix();
    glPushMatrix();
    glScalef(0.7 * 1, 0.7 * 1.2, 1.0);
    glTranslatef(0 - 0.8, 0, 0);
...
void hat() {
    //up
    glColor3f(0.8f, 0.5f, 0.1f);
    drawHalfdownCircle(-0.5f, 0.23f, 0.1);
    //bottom
    glLineWidth(18);
    glBegin(GL_LINES);
    glColor3f(0.8f, 0.5f, 0.1f);
...
//----------******************* SCENE TRANSITIONING AND DISPLAY PART ******************--------
void display()
{
